...
public:
	...
	size_t blank_shots{ 10 }; //to evaluate influence of memoization 
	...
	Matrix <double, Dynamic, Dynamic> w1X;
	Matrix <double, Dynamic, Dynamic> grw1X;
	...
	)
	{
		...
		w1X.resize(n1, n_train);
		grw1X.resize(n1, n_train);
	...
	//Here t>0 and the product grw1X = gr_w1 * X_train should be stored beforehand
	double fast_forward_propagation(const double& t)
	{
		double fx(0.0);
		accuracy = 0.;
		size_t maxInd{};
		...
		z1 = (w1X - t * grw1X).colwise() + (b1-t*gr_b1);	
		a1 = z1.array().cwiseMax(0).pow(2);
		z2 = ((w2-t*gr_w2) * a1).colwise() + (b2-t*gr_b2);
	...
	void update_params(const double& t)
	{
		w1 -= t * gr_w1;
		b1 -= t * gr_b1;
		w2 -= t * gr_w2;
	...
	{
		initialize();
		//w1X - will be used later to talculate z1 = (w1X - t * grw1X) * X_train . . . ;
		w1X = w1 * X_train;
		for (size_t i{ 0 }; i < iterations; ++i)
		{
			objective_function = forward_propagation();
			backward_propagation();
			//grw1X  - will be used later to talculate z1 = (w1X - t * grw1X) * X_train;
			grw1X = gr_w1 * X_train;
			w1X -= learning_rate * grw1X;
	...
	void gradient_descent_with_blank_shots()
	{
		initialize();
		//w1X - will be used later to talculate z1 = (w1X - t * grw1X) * X_train . . . ;
		w1X = w1 * X_train;
		for (size_t i{ 1 }; i < iterations; ++i) 
		{
			...
			for (size_t k{ 0 }; k < blank_shots; ++k)
			{
				double t = (1e-5*rand() + 0.) / RAND_MAX;
				cout << "    t=" << t << ", Blank_shot " << k << ", f(x-tg)=: " << fast_forward_propagation(t) << endl;
			}
			fast_forward_propagation(0.);
...
void gradient_descent_with_blank_shots()
{
	...
	p->gradient_descent_with_blank_shots();	 
}
